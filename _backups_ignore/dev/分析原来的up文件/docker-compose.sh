...
function canonicalize_data_paths() {
  ...
  if [[ "${OVERLEAF_LOG_PATH:-null}" != "null" ]]; then
    OVERLEAF_LOG_PATH=$(cd "$TOOLKIT_ROOT"; realpath "$OVERLEAF_LOG_PATH")      ### THIS
  fi
  MONGO_DATA_PATH=$(cd "$TOOLKIT_ROOT"; realpath "$MONGO_DATA_PATH")        ### THIS
  REDIS_DATA_PATH=$(cd "$TOOLKIT_ROOT"; realpath "$REDIS_DATA_PATH")        ### THIS    
  GIT_BRIDGE_DATA_PATH=$(cd "$TOOLKIT_ROOT"; realpath "$GIT_BRIDGE_DATA_PATH")      ### THIS
}
...
function set_base_vars() {
  ...
  if [[ "$IMAGE_VERSION_MAJOR" -lt 5 ]]; then
    ...
    DOCKER_COMPOSE_FLAGS+=(-f "$TOOLKIT_ROOT/lib/docker-compose.vars.yml")        ########### THIS 5.5
  fi
  ...
  then
    ...
    OVERLEAF_LISTEN_IP="0.0.0.0"                       ########### THIS 5.5
  fi
  ...
  OVERLEAF_IN_CONTAINER_DATA_PATH=/var/lib/overleaf       ########### THIS 5.5
  if [[ "$IMAGE_VERSION_MAJOR" -lt 5 ]]; then
  ...
  export OVERLEAF_PORT        ##### 80
  export OVERLEAF_IN_CONTAINER_DATA_PATH #####/var/lib/overleaf
...
function set_redis_vars() {
  ...
  elif [[ $REDIS_AOF_PERSISTENCE == "true" ]]; then       #### THIS
...
read_image_version
read_mongo_version
read_config     # GIT_BRIDGE_ENABLED=false  ， Then source "$TOOLKIT_ROOT/config/overleaf.rc"
check_retracted_version
check_sharelatex_env_vars
build_environment     # canonicalize_data_paths， set_base_vars，set_redis_vars，set_mongo_vars，       set_sibling_containers_vars，set_logging_vars，set_nginx_vars，set_git_bridge_vars
print_debug_info "$@"
docker_compose "$@"
